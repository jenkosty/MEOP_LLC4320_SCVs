...
LLC = LLCcyclones_optimized;
MEOP = MEOPcyclones_optimized;
...
MEOPdata = NaN(length(depth_grid), length(MEOP));
for ii = 1:length(MEOP)
    %plot(MEOP(ii).dha_anom_profile_fitted, MEOP(ii).dha_pres_profile_fitted, 'Color', [0.5 0.5 0.5], 'LineWidth', 1);
...
MEOPdata = NaN(length(depth_grid), length(MEOP));
for ii = 1:length(MEOP)
    %plot(MEOP(ii).N2_anom_profile, MEOP(ii).N2_pres_profile, 'Color', [0.5 0.5 0.5], 'LineWidth', 1);
    ind = ~isnan(MEOP(ii).N2_anom_profile) & ~isnan(MEOP(ii).N2_pres_profile);
    MEOPdata(:,ii) = interp1(MEOP(ii).N2_pres_profile(ind), MEOP(ii).N2_anom_profile(ind), depth_grid);
end
mean_profile = mean(MEOPdata, 2, 'omitnan');
mean_profile(isnan(mean_profile)) = 0;
rmse_profile = rmse(MEOPdata, mean_profile, 2, 'omitnan');
rmse_profile(isnan(rmse_profile)) = 0;
%plot(mean_profile - rmse_profile, depth_grid, 'r--', 'LineWidth', 3, 'DisplayName', 'MEOP');
%plot(mean_profile + rmse_profile, depth_grid, 'r--', 'LineWidth', 3, 'DisplayName', 'MEOP');
x_fill = vertcat(mean_profile+rmse_profile, flipud(mean_profile-rmse_profile)); 
y_fill = vertcat(depth_grid, flipud(depth_grid)); 
fill(x_fill, y_fill, MEOPclr, 'FaceAlpha', alpha, 'LineStyle', 'none'); 
a = plot(smooth(mean(MEOPdata(:,strcmp([MEOP.type], "deep")), 2, 'omitnan'), 10), depth_grid, 'Color', MEOPclr,  'LineWidth', lw, 'DisplayName', 'MEOP');
plot(smooth(mean(MEOPdata(:,strcmp([MEOP.type], "shallow")), 2, 'omitnan'), 10), depth_grid, '--', 'Color', MEOPclr, 'LineWidth', lw, 'DisplayName', 'MEOP');

%%% LLC
LLCdata = NaN(length(depth_grid), length(LLC));
for ii = 1:length(LLC)
    %plot(LLC(ii).dha_anom_profile_fitted, LLC(ii).dha_pres_profile_fitted, 'Color', [0.5 0.5 0.5], 'LineWidth', 1)
    ind = ~isnan(LLC(ii).N2_anom_profile) & ~isnan(LLC(ii).N2_pres_profile);
    LLCdata(:,ii) = interp1(LLC(ii).N2_pres_profile(ind), LLC(ii).N2_anom_profile(ind), depth_grid);
end
mean_profile = mean(LLCdata, 2, 'omitnan');
mean_profile(isnan(mean_profile)) = 0;
rmse_profile = rmse(LLCdata, mean_profile, 2, 'omitnan');
...
y_fill = vertcat(depth_grid, flipud(depth_grid)); 
fill(x_fill, y_fill, LLCclr, 'FaceAlpha', alpha, 'LineStyle', 'none'); 
b = plot(smooth(mean(LLCdata(:,strcmp([LLC.type], "deep")), 2, 'omitnan'), 10), depth_grid, 'Color', LLCclr, 'LineWidth', lw, 'DisplayName', 'LLC4320');
plot(smooth(mean(LLCdata(:,strcmp([LLC.type], "shallow")), 2, 'omitnan'), 10), depth_grid, '--', 'Color', LLCclr, 'LineWidth', lw, 'DisplayName', 'LLC4320');
...
if save_fig == 1
    exportgraphics(f,'Paper Figures/cyclone_profiles.png','Resolution',600)
end
